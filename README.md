...
권한 숫자는 **소유자 / 그룹 / 기타 사용자** 순서이며, 각 숫자는 아래 합으로 계산한다.
...
- `755`는 디렉토리나 실행 파일에 자주 사용되며, 소유자는 읽기·쓰기·실행이 가능하고 나머지는 읽기·실행만 가능하다.
- `644`는 일반 텍스트 파일에 자주 사용되며, 소유자는 읽기·쓰기 가능, 나머지는 읽기만 가능하다.

### 6-3. 왜 파일과 디렉토리에 다르게 적용했는가

- 파일 `test.txt`는 실행이 필요 없는 일반 텍스트 파일이므로 `644`가 적절하다.
- 디렉토리 `practice`는 내부 접근이 가능해야 하므로 실행 비트가 포함된 `755`를 주는 것이 일반적이다.
